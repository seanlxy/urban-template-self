   ...
   if($_POST['action'] === 'restore')
    {
        ...
        if(count($items_to_restore) > 0)
        {
            ...
            $query = "UPDATE `beamer_campaign` SET `status` = 'H',`date_deleted` = NULL, `deleted_by`='0' WHERE `id` IN ($page_ids)";
    ...
    function generate_item_list($parent_id = 0)
    {
        ...
        $sql = "SELECT `id`, `name`, DATE_FORMAT(`date_deleted`, '%d %b %Y %r') AS deleted_date
