   ...
   if($_POST['action'] === 'restore')
    {
        ...
        if(count($items_to_restore) > 0)
        {
            ...
            $query = "UPDATE `form` SET `status` = 'H', `date_deleted` = NULL WHERE `id` IN ($page_ids)";
    ...
    function generate_item_list($parent_id = 0)
    {
        ...
        $sql = "SELECT `id`, `name`,
            IF(`date_deleted`, DATE_FORMAT(`date_deleted`, '%d %b %Y %h:%i %p'), '-') AS deleted_on
            FROM `form`
            WHERE `status` = 'D'";
        ...
        <table width="100%" class="bordered">
             <thead>
                <tr>
                    ...
                    <th  width="150" align="left">Deleted On</th>
